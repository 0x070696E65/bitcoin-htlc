...
export class btcSwap {
    ...
    public async lock(lockHeight: number, sender: ECPairInterface, receiver: ECPairInterface, amount: number, fee: number){
        ...
        const p2wsh = bitcoin.payments.p2wsh({redeem: {output: swapContract, network: this.network}, network: this.network})
        ...
        if(senderAddress == undefined || contractAddress == undefined) throw new Error("senderAddress or contractAddress is undefined")
        // UTXOを取得
        ...
        // トランザクションHEXを作成
        const txHex = this.coin.buildAndSignTx(
            sender,
            senderAddress,
            this.network,
            contractAddress,
        ...
        this.coin.postTransaction(txHex)
        .then(result=>console.log('Transaction hash:', result))
        .catch(err=>console.error(err.response.data))
    }

    async withdraw(txId: string, contractAddress: string, witnessScript: string, receiver: ECPairInterface, preImage: string, feeSat: number){
    ...
    swapContractGenerator(receiverPublicKey: Buffer, userRefundPublicKey: Buffer, PAYMENT_HASH: string, timelock: number) {
